...
async function currentEstate(db:D1Database):Promise<Record<string,unknown>|null>{
  return db.prepare(`SELECT estate_revision_id, source_set_hash, source_revision_ids_json, built_at, activated_at, quality_json
...
export async function handleIntegratedRoutes(request:Request,env:IntegratedRoutesEnv):Promise<Response|null>{
  ...
  try{
    const estate=await currentEstate(env.DB); if(!estate)return null; const estateId=String(estate.estate_revision_id);
