#!/usr/bin/env python3
"""Read-only IBM MQ telemetry observer skeleton.

The observer emits `osi.telemetry.batch/v1` JSON. It only executes an explicit
allowlist of MQSC DISPLAY commands plus `dspmq` discovery when queue managers are
not supplied. It does not consume messages, alter MQ state, or publish anywhere.
"""
...
class CommandResult:
    ...
    def evidence_ref(self) -> str:
        return f"telemetry://{self.sample_id}/{self.qmgr}/{self.family}"
...
def run_mqsc(runmqsc: str, qmgr: str, family: str, sample_id: str, observed_at: str, timeout: int) -> CommandResult:
...
def observation_id(source_id: str, semantic_type: str, hints: dict[str, str], metric: str, sample_id: str, discriminator: str = "") -> str:
    stable_hints = json.dumps(hints, sort_keys=True, separators=(",", ":"))
    raw = "|".join([source_id, semantic_type, stable_hints, metric, sample_id, discriminator])
...
def make_observation(
    ...
    *,
    ...
    item: dict[str, Any] = {
        "observation_id": observation_id(source_id, semantic_type, hints, metric, result.sample_id, discriminator),
        "entity": {
...
def observations_from_result(source_id: str, source_host: str, qmid: str | None, result: CommandResult) -> list[dict[str, Any]]:
    ...
    if result.family == "qmgr_status":
        for values in rows:
            ...
            out.append(make_observation(source_id, source_host, result, "mq.queue_manager", result.qmgr, hints, "mq.queue_manager.status", status, "state"))
        return out
        ...
        for values in rows:
            ...
            for attr, metric, unit in QUEUE_METRICS:
                ...
                if value is not None:
                    out.append(make_observation(source_id, source_host, result, "mq.queue", name, hints, metric, value, unit))
        return out
    ...
    if result.family == "listener_status":
        for values in rows:
            ...
            status = values.get("STATUS") or "UNKNOWN"
            hints = identity_hints(result.qmgr, qmid, name)
            out.append(make_observation(source_id, source_host, result, "mq.listener", name, hints, "mq.listener.status", status, "state"))
        return out
...
def coverage_from_result(result: CommandResult) -> dict[str, Any]:
    ...
    if result.state != "point_in_time":
        ...
        error = "; ".join(bits) or "MQSC collection failed"
    return {
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    identity_at = utc_now()
    for qmgr in qmgrs:
        result = run_mqsc(args.runmqsc, qmgr, "qmgr_identity", "identity", identity_at, args.timeout)
    ...
    for sample_index in range(1, args.samples + 1):
        ...
        for qmgr in qmgrs:
            for family in RUNTIME_FAMILIES:
                result = run_mqsc(args.runmqsc, qmgr, family, sample_id, observed_at, args.timeout)
                coverage.append(coverage_from_result(result))
