#!/usr/bin/env python3
from __future__ import annotations

import json
import tempfile
import unittest
from pathlib import Path
from unittest import mock

import telemetry_delivery as delivery

SECRET = "s" * 32


def sample_batch() -> dict:
    return {
        "schema_version": "osi.telemetry.batch/v1",
        "run": {
            "run_id": "telemetry_test",
            "observer": "osi-mq-observer",
            "observer_version": "0.2.0",
            "profile": "baseline",
            "started_at": "2026-09-11T00:00:00Z",
            "completed_at": "2026-09-11T00:00:01Z",
            "sample_count": 1,
            "sample_interval_seconds": 60,
            "source": {"kind": "ibm_mq_host", "source_id": "mq-a.example", "source_host": "mq-a"},
        },
        "coverage": [],
        "observations": [],
    }


class DeliveryTests(unittest.TestCase):
    def test_spool_is_content_idempotent_across_json_whitespace(self):
        batch = sample_batch()
        raw_a = json.dumps(batch, indent=2).encode()
        raw_b = json.dumps(batch, separators=(",", ":")).encode()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            did_a, created_a = delivery.spool_batch(raw_a, root)
            did_b, created_b = delivery.spool_batch(raw_b, root)
            self.assertEqual(did_a, did_b)
            self.assertTrue(created_a)
            self.assertFalse(created_b)
            self.assertEqual(delivery.spool_status(root), {"pending": 1, "sent": 0, "dead": 0})

    def test_signature_binds_payload_key_delivery_and_timestamp(self):
        payload = delivery.canonical_json_bytes(sample_batch())
        headers = delivery.signed_headers(payload, "observer-a", SECRET, "1000")
        ok, reason = delivery.verify_signature(payload, headers, SECRET, now=1000)
        self.assertTrue(ok, reason)

        tampered = payload + b"\n"
        ok, reason = delivery.verify_signature(tampered, headers, SECRET, now=1000)
        self.assertFalse(ok)
        self.assertEqual(reason, "content_hash_mismatch")

        changed_key = dict(headers)
        changed_key["X-OSI-Key-Id"] = "observer-b"
        ok, reason = delivery.verify_signature(payload, changed_key, SECRET, now=1000)
        self.assertFalse(ok)
        self.assertEqual(reason, "signature_mismatch")

        ok, reason = delivery.verify_signature(payload, headers, SECRET, now=1400, max_skew_seconds=300)
        self.assertFalse(ok)
        self.assertEqual(reason, "timestamp_outside_window")

    def test_weak_secret_and_unsafe_key_id_are_rejected(self):
        payload = delivery.canonical_json_bytes(sample_batch())
        with self.assertRaises(delivery.DeliveryError):
            delivery.signed_headers(payload, "observer-a", "short", "1000")
        with self.assertRaises(delivery.DeliveryError):
            delivery.signed_headers(payload, "observer-a\nInjected: yes", SECRET, "1000")

    def test_failed_send_stays_pending_with_backoff(self):
        raw = json.dumps(sample_batch()).encode()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            did, _ = delivery.spool_batch(raw, root)
            with mock.patch.object(delivery, "post_payload", side_effect=delivery.DeliveryError("network down")):
                summary = delivery.publish_pending(root, "https://telemetry.example/ingest", "observer-a", SECRET, now_epoch=1000)
            self.assertEqual(summary["failed"], 1)
            self.assertEqual(delivery.spool_status(root)["pending"], 1)
            record = delivery.read_record(root / "pending" / f"{did}.json")
            self.assertEqual(record["attempts"], 1)
            self.assertGreater(record["next_attempt_epoch"], 1000)
            self.assertIn("network down", record["last_error"])

    def test_successful_send_moves_record_to_sent(self):
        raw = json.dumps(sample_batch()).encode()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            did, _ = delivery.spool_batch(raw, root)
            result = delivery.PostResult(202, '{"status":"accepted"}')
            with mock.patch.object(delivery, "post_payload", return_value=result):
                summary = delivery.publish_pending(root, "https://telemetry.example/ingest", "observer-a", SECRET, now_epoch=1000)
            self.assertEqual(summary["sent"], 1)
            self.assertFalse((root / "pending" / f"{did}.json").exists())
            sent = delivery.read_record(root / "sent" / f"{did}.json")
            self.assertEqual(sent["last_status"], 202)
            self.assertEqual(sent["attempts"], 1)

    def test_duplicate_pending_is_healed_when_sent_record_already_exists(self):
        raw = json.dumps(sample_batch()).encode()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            did, _ = delivery.spool_batch(raw, root)
            pending = root / "pending" / f"{did}.json"
            record = delivery.read_record(pending)
            delivery.atomic_write_json(root / "sent" / f"{did}.json", record)
            with mock.patch.object(delivery, "post_payload") as post:
                summary = delivery.publish_pending(root, "https://telemetry.example/ingest", "observer-a", SECRET, now_epoch=1000)
            post.assert_not_called()
            self.assertEqual(summary["already_sent"], 1)
            self.assertFalse(pending.exists())

    def test_endpoint_validation_requires_https_or_exact_loopback(self):
        self.assertEqual(
            delivery.validate_endpoint("http://localhost:8787/ingest"),
            "http://localhost:8787/ingest",
        )
        self.assertEqual(
            delivery.validate_endpoint("http://[::1]:8787/ingest"),
            "http://[::1]:8787/ingest",
        )
        with self.assertRaises(delivery.DeliveryError):
            delivery.validate_endpoint("http://telemetry.example/ingest")
        with self.assertRaises(delivery.DeliveryError):
            delivery.validate_endpoint("http://localhost.evil.example/ingest")
        with self.assertRaises(delivery.DeliveryError):
            delivery.validate_endpoint("https://user:pass@telemetry.example/ingest")
        with self.assertRaises(delivery.DeliveryError):
            delivery.validate_endpoint("https://telemetry.example/ingest#fragment")

    def test_redirect_handler_does_not_follow_redirects(self):
        handler = delivery._NoRedirectHandler()
        self.assertIsNone(handler.redirect_request(None, None, 307, "redirect", {}, "https://other.example/"))


if __name__ == "__main__":
    unittest.main()
