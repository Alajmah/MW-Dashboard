...
def observation(semantic_type: str, display_name: str, hints: dict, obs_id: str = "tobs_111111111111111111111111") -> dict:
    return {
        ...
        "source": {
            ...
            "evidence_ref": "telemetry://sample-0001/QM1/queue_status",
...
class ResolverTests(unittest.TestCase):
    ...
    def test_conflicted_canonical_entity_is_not_indexed(self):
        snap = snapshot()
        bad = copy.deepcopy(snap)
        bad["entities"][1]["identity_state"] = "conflicted"
        item = observation("mq.queue", "Q.TEST.OUT", {"queue_manager_name": "QM1", "name": "Q.TEST.OUT"})
        result = resolver.resolve_batch(batch([item]), bad)
        self.assertEqual(result["quarantine"][0]["reason"], "scoped_identity_not_found")
